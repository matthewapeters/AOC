...
def search_directions(
    ...
    # we only bother to search if we can search in all four diagonal directions
    if len(directions) == 4:
        return directions
    return []
...
def search(
    ...
    """
    # only consider x,y if it is the (first) second letter of the target
    if data[y][x] != target[1]:
        return 0
    radius = int(length / 2 + 0.5)
    directions = search_directions(x, y, radius, limits)
    if VALIDATE:
        found = {
            d: all(
                (
