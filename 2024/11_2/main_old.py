...
def blink_a_bunch(
    blinks: int, rock_list: List[int], perm_map: Dict[int, List[int]]
) -> List[int]:
    """blink_a_bunch"""
    blinks -= 1
    if blinks == 0:
        return rock_list
    rock_list = [output for rock in rock_list for output in perm_map[rock]]
    return blink_a_bunch(blinks, rock_list, perm_map)
...
if __name__ == "__main__":
    ...
    rock_set: List[int] = list(set(rocks))
    pool_size: int = 5
    ...
    with Pool(pool_size) as p:
        for i in range(blink_times):
            ...
            chunk: Dict[int, List[int]] = {
                input: outputs
                for results in p.map(blink, rock_set)
                for input, outputs in results.items()
                if input not in permutation_map
            }
            print()
            ...
            rock_set = list(
                set(
                    [
                        ...
                        if output not in permutation_map
                    ]
                )
            )
            permutation_map.update(chunk)

        print(f"apply {blink_times} iterations of changes to rocks")
        total_rocks: int = 0
