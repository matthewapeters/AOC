...
# ---------------------
#
# When checking extents, we only need to
# check length -1, because the first letter
# occupies a valid spot, so we are only
# validating the positions for remaining characters
#
#  5
#  6S     S
#  7 A   A
...
def search_directions(
    ...
    return [
        ...
        for direction, extent in {
            d: (x + (d[0] * length), y + (d[1] * length)) for d in SEARCH_DIRECTIONS
        }.items()
