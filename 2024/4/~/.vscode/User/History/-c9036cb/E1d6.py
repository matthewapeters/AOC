...
def search_directions(
    ...
    return [
        ...
        for direction, extent in {
            t: (x + (t[0] * length), y + (t[1] * length)) for t in SEARCH_DIRECTIONS
        }.items()
...
def search(
    ...
    if VALIDATE:
        found = {
            ...
            # we do not need to consider X, so length is one less
            for d in search_directions(x, y, length, limits)
        }
        for d in [d for d, f in found.items() if f]:
            # we want to cover full length, add 1
...
"""
    ...
    for y in range(len_y):
        for x in range(len_x):
            ...
            found += search(TARGET, data, x, y, length - 1, limits, proof)
