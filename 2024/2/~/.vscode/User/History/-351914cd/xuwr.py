...
def evaluate_report(report: List[int])->bool:
    ...
    for i in range(len(report)-1):
        ...
        if abs(diff) not in (1,2,3):
            print(f"report: {report}\tindex: {i}\tdiff: {diff}")
            return False
        direction = 1 if diff>0 else -1 if diff<0 else 0
        if direction == 0:
            print(f"report: {report}\tindex: {i}\tdirection=0")
            return False
        if last_direction == 0:
            last_direction = direction
        else:
            if last_direction != direction:
                print(f"report: {report}\tindex: {i}\tlast_direction: {last_direction}\tdirection: {direction}")
                return False
    return True
