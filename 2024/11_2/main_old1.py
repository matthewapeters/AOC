...
PERMUTATION_MAP: Dict[Tuple[int, int], List[int]] = {}


def blink_a_bunch(blinks: int, rock_list: List[int]) -> List[int]:
    """blink_a_bunch"""
    blinks -= 1
    if blinks == 0:
        return rock_list
    rock_list = [output for rock in rock_list for output in PERMUTATION_MAP[rock]]
    return blink_a_bunch(blinks, rock_list)


@cache
def blink(n: int) -> Dict[int, List[int]]:
    """blink"""
    ...
    if len(f"{n}") % 2 == 0:
        split = int(len(f"{n}") / 2)
        return {n: [int(f"{n}"[:split]), int(f"{n}"[split:])]}
...
if __name__ == "__main__":
    ...
    chunk_size: int = 8000000  # 40000000
    pool_size: int = 5

    print(f"compute changes over {blink_times} blinks")
    with Pool(pool_size) as p:
        for rock in rocks:
            rock_set: List[int] = [rock]
            for i in range(blink_times):
                print(
                    datetime.now() - last_time,
                    i,
                    pool_size,
                    f"{len(rocks):5} -> {len(rock_set):5}",
                    len(PERMUTATION_MAP),
                    end="",
                )
                last_time = datetime.now()
                # compute change
                raw_results = p.map(
                    blink, [r for r in rock_set if (r, 1) not in PERMUTATION_MAP]
                )
                chunk: Dict[int, List[int]] = {
                    (input, 1): outputs
                    for results in raw_results
                    for input, outputs in results.items()
                    if (input, 1) not in PERMUTATION_MAP
                }
                print()
                # replace rock_set with distinct new rock values that
                # have not been seen before
                rock_set = list(
                    set(
                        [
                            output
                            for outputs in chunk.values()
                            for output in outputs
                            if (output, 1) not in PERMUTATION_MAP
                        ]
                    )
                )
                PERMUTATION_MAP.update(chunk)
                if (rock, i + 1) not in PERMUTATION_MAP:
                    PERMUTATION_MAP[(rock, i + 1)] = [
                        r
                        for rx in PERMUTATION_MAP[(rock, i)]
                        for r in PERMUTATION_MAP[(rx, 1)]
                    ]
